...
setInterval(() => {
		...
		xhr.onload = () => {
			if (xhr.status == 200) {
				...
				document.getElementById("hostbtn").innerText =
					dict["udid"] == udid ? "Hostになリました" : "Hostになる";
	...
	if (document.getElementById("clickButton").checked == true) {
		let clickCancel = document.querySelectorAll(
	...
	let nextEpisode = document
		.querySelectorAll(".com-content-list-ContentListEpisodeItem__overview")
		[currentEpisode].querySelector("a")
		.getAttribute("href");
